...
class WebpackConfig {
    ...
    enableSvelte(options = {}) {
        ...
        for (const optionKey of Object.keys(options)) {
            if (!(optionKey in this.svelteOptions)) {
                throw new Error(`Invalid option "${optionKey}" passed to enableSvelte(). Valid keys are ${Object.keys(this.svelteOptions).join(', ')}`);
            }

            this.svelteOptions[optionKey] = options[optionKey];
        }
    }
